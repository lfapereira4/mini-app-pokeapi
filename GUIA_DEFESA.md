...
É um detalhe decorativo, sem relação com os requisitos da atividade: quando o ID do personagem está na lista `IDS_COM_POKEMON` (os números de 1 a 10, mais 10 lendários — Articuno, Zapdos, Moltres, Mewtwo, Mew, Lugia, Ho-Oh, Rayquaza, Dialga e Palkia), a função `exibirResultado()` troca a imagem exibida por uma foto de Pokémon e mostra um botão "🔊 Ouvir" com o grito dele. Esses arquivos (imagem e áudio) foram baixados uma única vez do repositório oficial da PokéAPI no GitHub e ficam salvos localmente em `public/img/pokemon/` e `public/audio/pokemon/` — não é uma chamada de API acontecendo a cada busca, é um arquivo estático já salvo no projeto. Os DADOS continuam sendo os do personagem de verdade (nome, espécie, status, gênero, origem) — só a imagem/som muda, e só para fins visuais. A lógica de consumo da API (fetch, async/await, tratamento de erro) não foi alterada por causa disso.

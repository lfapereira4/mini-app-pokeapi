...
## Pergunta extra: "por que aparece foto (e som) de Pokémon em alguns personagens?"
É um detalhe decorativo, sem relação com os requisitos da atividade: quando o ID do personagem está na lista `IDS_COM_POKEMON` (os números de 1 a 10, mais 10 lendários — Articuno, Zapdos, Moltres, Mewtwo, Mew, Lugia, Ho-Oh, Rayquaza, Dialga e Palkia), a função `exibirResultado()` troca a imagem exibida por um sprite ANIMADO (gif) do Pokémon e mostra um botão "🔊 Ouvir" com o grito dele. Esses arquivos (imagem e áudio) foram baixados uma única vez do repositório oficial da PokéAPI no GitHub (`PokeAPI/sprites`, sprites animados da geração V) e ficam salvos localmente em `public/img/pokemon-animado/` e `public/audio/pokemon/` — não é uma chamada de API acontecendo a cada busca, são arquivos estáticos já salvos no projeto. Os DADOS continuam sendo os do personagem de verdade (nome, espécie, status, gênero, origem) — só a imagem/som muda, e só para fins visuais. A lógica de consumo da API (fetch, async/await, tratamento de erro) não foi alterada por causa disso.

Importante: esses 20 IDs (1-10 e os 10 lendários) são os números do PERSONAGEM na Rick and Morty API, não o nome do Pokémon — ou seja, pra ver o Mewtwo, por exemplo, é preciso digitar `150` na busca (o ID do personagem #150 da Rick and Morty API), não escrever "mewtwo" (que não existe como personagem nessa API).

## Pergunta extra: "o fundo com pokébolas também é da API?"
Não — é um SVG (desenho vetorial) feito direto no CSS, repetido como padrão de fundo. Não baixa nada da internet nem depende de nenhuma API; é só estética.
